...
export function RealtimeVillagePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = params;
  return <VillageDetail slug={slug} />;
}

function VillageDetail({ slug }: { slug: string }) {
  const [result, setResult] = useState<Awaited<ReturnType<typeof getVillage>>>(null);

  async function load() {
    const data = await getVillage(slug);
    setResult(data);
  }

  useRealtimeVillages(load);
