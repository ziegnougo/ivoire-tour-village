...
export default function EditOffrePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: villagesData } = await supabase.from("villages").select("id, nom").order("nom", { ascending: true });
  ...
  if (!offre) return <div>Offre introuvable</div>;

  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900">Modifier l'offre</h1>

      <form action={saveOffre} className="mt-8 space-y-6 max-w-2xl">
        <input type="hidden" name="id" defaultValue={offre.id} />
